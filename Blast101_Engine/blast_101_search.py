...
def process_blast(myline_database):
   ...
   #add some BLAST  stuff here
    ...
    for el in range(1,len(res)):
        if res[el][0]==res_store[cnt][0]: #bug HERE defaultdict(<class 'list'>, {0: [(0, 0)], 1: [(0, 1)], 2: [(0, 2)]})
            res_store[cnt].append(res[el])
        else:
            cnt+=1
            res_store[cnt].append(res[el])
    #print(res_store) -here
...
def process_fasta_file():
    ...
    res = pff.process_fasta_file(args.db, process_blast, max_scores,aligner_timer_secs)
...
def print_final_results(res):
    ...
    print("Matrix: Blosum%s, Gap Weight: %s" % (args.blosum, args.gap))
    ...
    with open(r"logs\BLsearch.csv", 'w') as outputfile:
